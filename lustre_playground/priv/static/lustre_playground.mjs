...
function append3(first5, second2) {
  let _pipe = first5;
...
function new_seed(from2) {
  const [state, step2] = next([0, 1013904223]);
  const new_state = state + from2 >>> 0;
  return next([new_state, step2]);
}
function next(seed) {
  const [state, step2] = seed;
  const new_state = state * 1664525 + step2 >>> 0;
  return [new_state, step2];
}
...
function step(generator, seed) {
  return generator.step(seed);
}
function sort_ascending(one, other, compare3) {
  let $ = compare3(one, other);
  if ($ instanceof Lt) {
    return [one, other];
...
function int2(from2, to2) {
  return new Generator(
    (seed) => {
      let $ = sort_ascending(from2, to2, compare2);
      let low = $[0];
      let high = $[1];
...
var Model2 = class extends CustomType {
  constructor(count, target, history) {
    super();
    this.count = count;
    this.target = target;
...
var ApiIncrementBy = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var ApiDecrementBy = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
function init2(_) {
  return [new Model2(0, 10, toList([])), none()];
}
function int_list_to_string(int_list) {
  return fold(
    reverse(int_list),
    "[]",
    (current_string, new_int) => {
      let _pipe = current_string;
      let _pipe$1 = drop_right(_pipe, 1);
      return append3(_pipe$1, " " + to_string2(new_int) + " ]");
    }
  );
}
function view(model) {
  let count_string = to_string2(model.count);
  ...
  let target_message = (() => {
    ...
    if ($ === $1) {
      ...
      let target = $1;
      return "Target : " + to_string2(target);
    }
  })();
  return div(
    ...
    toList([
      div(
        ...
        toList([
          header(
            ...
            toList([
              h1(
                toList([class$("w-full mx-auto max-w-screen-xl font-bold")]),
                toList([text2("FairShare")])
              )
            ])
          )
        ])
      ...
      button(
        toList([class$("p-2"), on_click(new Increment())]),
        toList([text("+")])
      ),
      button(
        toList([class$("p-2"), on_click(new Decrement())]),
        toList([text("-")])
      ),
      h2(toList([]), toList([text2("Current Count")])),
      text2(count_string),
      h2(toList([]), toList([text2("History")])),
      text2(count_history),
      br(toList([])),
      br(toList([])),
      text2(target_message)
    ])
  );
}
function get_random_number(increment) {
  let randint_generator = int2(1, 2);
  let $ = step(randint_generator, random2());
  let roll_result = $[0];
  return from(
    (dispatch) => {
      if (increment) {
        return dispatch(new ApiIncrementBy(new Ok(toList([roll_result]))));
      } else {
        return dispatch(new ApiDecrementBy(new Ok(toList([roll_result]))));
      }
    }
  );
}
function update(model, msg) {
  ...
  if (msg instanceof Increment) {
    return [model, get_random_number(true)];
  } else if (msg instanceof Decrement) {
    return [model, get_random_number(false)];
  } else if (msg instanceof ApiIncrementBy && msg[0].isOk()) {
    let int_list = msg[0][0];
    if (int_list.atLeastLength(1)) {
      let number = int_list.head;
      return [
        model.withFields({
          count: model.count + number,
          history: prepend(number, model.history)
        }),
        none()
      ];
    } else {
  ...
  } else if (msg instanceof ApiDecrementBy && msg[0].isOk()) {
    let int_list = msg[0][0];
    if (int_list.atLeastLength(1)) {
      let number = int_list.head;
      return [
